...
/**
 * Manager-facing reservation application service. Coordinates guards, nights calculation, inventory
 * adjustments, status transitions, and orchestration for cancelation. All methods run within a
...
public class ManagerReservationService {
  ...
  /**
   * Entity-boundary guards to ensure hotel, room type, room, and reservation belong to the expected
   * scope.
   */
  private final EntityGuards entityGuards;
  /**
   * Domain service for validating and executing reservation status transitions with history.
   */
  private final ReservationStatusService statusService;
  /**
   * Orchestrator for higher-level flows such as cancelation (inventory release then status
   * change).
   */
  private final ReservationOrchestrator orchestrator;

  /**
   * Lists reservations for a hotel with optional filters. If both status and date range are
   * provided, both filters apply. If only date range is provided, the list is filtered by stay
   * range. If only status is provided, the list is filtered by status. If neither is provided, all
   * reservations of the hotel are returned.
   *
   ...
   * @throws NotFoundException if the hotel does not exist
   */
   ...
   * @throws NotFoundException   if reservation or hotel is not found
   * @throws BadRequestException if the reservation does not belong to the hotel
   */
  ...
  /**
   * Patches a reservation for a hotel manager. Supports: - Changing room type: releases old
   * inventory for the original dates, sets the new room type, and reserves inventory for the same
   * date range under the new room type. - Changing room id: verifies room belongs to hotel and
   * type, then sets it. - Changing dates: releases old inventory, recalculates nights and updates
   * dates, then reserves inventory for the new date range. - Updating scalar fields: numGuests,
   * currency, priceTotal, notes. - Updating status: uses status service to validate transition and
   * write history. Inventory operations are executed around date and room-type changes to maintain
   * consistency.
   *
   ...
   * @throws NotFoundException   if the reservation or hotel is not found
   * @throws BadRequestException if any ownership, status transition, or validation rule is
   *                             violated
   */
  ...
  /**
   * Applies a room-type upgrade for the reservation if upgrade status allows it. Flow: release
   * inventory for the current room type and dates, set the new room type, reserve inventory for the
   * same dates under the new type, set upgrade status and timestamp, then persist the reservation.
   ...
   * @throws NotFoundException   if hotel or reservation is not found
   * @throws BadRequestException if the reservation is not eligible for upgrade
   */
  ...
  /**
   * Checks in a reservation if it is not canceled and not already checked in. Delegates the
   * transition to the status service.
   *
   * @param hotelId       hotel that must own the reservation
   * @param reservationId reservation to check in
   * @return the reservation after the transition, or the original if already checked in
   * @throws NotFoundException   if hotel or reservation is not found
   * @throws BadRequestException if the reservation has been canceled
   */
  ...
  /**
   * Checks out a reservation if it is not canceled and not already checked out. Delegates the
   * transition to the status service.
   *
   * @param hotelId       hotel that must own the reservation
   * @param reservationId reservation to check out
   * @return the reservation after the transition, or the original if already checked out
   * @throws NotFoundException   if hotel or reservation is not found
   * @throws BadRequestException if the reservation has been canceled
   */
  ...
  /**
   * Cancels a reservation by delegating to the orchestrator (which releases inventory, records
   * cancelation timestamp, and changes status).
   *
   * @param hotelId       hotel that must own the reservation
   * @param reservationId reservation to cancel
   * @param reason        optional textual reason that will be stored by the orchestrator
   * @throws NotFoundException if hotel or reservation is not found
   */
